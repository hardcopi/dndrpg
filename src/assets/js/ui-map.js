...
(function () {
  ...
  function npcMarks(people, scale, opts) {
    ...
    const faces = shown.map((n, i) => {
      ...
      return `<g class="wm-npc" transform="translate(${at})"
        ...
        ${n.mark === 'offer' || n.mark === 'turnin' ? `<text class="wm-npc-mark"
          y="${(-r - 0.12).toFixed(2)}" dy="0"
          font-size="${(r * 1.7).toFixed(2)}">${n.mark === 'turnin' ? '?' : '!'}</text>` : ''}
      </g>`;
    }).reverse().join('');
  ...
  /**
   * A bite of floor through an open doorway into a room you have not entered.
   *
   * This is painted ON TOP of the rock-edge, not added to the ink union. A
   * stub in the union is an alcove: the outline goes around it and still
   * caps, so the hall still reads as closed. Covering the cap with paper
   * leaves a hole — floor going through the wall into the hatch, which is
   * the same fact first-person already frames. Only open and arch, and
   * only the end that is not already a walked room (that join is already
   * a gap in the union).
   */
  function openingStubs(plan, keep) {
    const stubs = [];
    const seenRooms = new Set(
      (plan.rooms || []).filter(keep).map((r) => r.location_id).filter((id) => id != null)
    );
    const h = HALL_W / 2;
    // Deep enough to swallow the wall line and a bite of hatch. A shallow
    // alcove still looks like a dead end; this is a hole you can see into.
    const deep = HALL_W * 0.9;
    const back = HALL_WALL;
    (plan.corridors || []).forEach((c) => {
      if ((c.door !== 'open' && c.door !== 'arch') || !keep(c)) return;
      const pts = c.points || [];
      ...
      ends.forEach(({ p, n, room }) => {
        if (room != null && seenRooms.has(room)) return;
        const vert = Math.abs(n[0] - p[0]) < 1e-6;
        if (vert) {
          const dir = p[1] >= n[1] ? 1 : -1;
          const yA = p[1] - dir * back;
          const yB = p[1] + dir * deep;
          stubs.push([p[0] - h, Math.min(yA, yB), p[0] + h, Math.max(yA, yB)]);
        } else {
          const dir = p[0] >= n[0] ? 1 : -1;
          const xA = p[0] - dir * back;
          const xB = p[0] + dir * deep;
          stubs.push([Math.min(xA, xB), p[1] - h, Math.max(xA, xB), p[1] + h]);
        }
      });
    });
    return stubs;
   ...
   *   open/arch  nothing hanging in the gap. The wall itself is the sign: a
   *              stub of floor is punched through the ink at the threshold so
   *              a hall into a room you have not entered is not a dead end.
   *
  ...
  function floorplan(plan, hereId, ways) {
    ...
    const openings = openingStubs(plan, lit);
    ...
    // An open threshold and an arch have no hanging glyph — the wall having
    // a hole is the sign, punched as `openings` into the ink union. They are
    // still a way through, and on a floor generated by DungeonGen they are
    // SIXTY PER CENT of the thresholds (see DungeonGen::door). They get the
    // same invisible pad the glyphs get, or those rooms would have no way in
    // that a pointer could find.
    ...
    /**
     * One dressing mark. Kept tiny: the plan already has a room shape, and
     * six millimetres of furniture would be a second map.
     */
    function dressGlyph(kind, cx, cy, r) {
      const x = round3(cx);
      const y = round3(cy);
      const a = round3(r);
      const b = round3(r * 0.55);
      switch (kind) {
        case 'pillar':
          return `<circle cx="${x}" cy="${y}" r="${a}"/>`;
        case 'well':
        case 'pool':
          return `<circle cx="${x}" cy="${y}" r="${a}"/><circle cx="${x}" cy="${y}" r="${b}"/>`;
        case 'altar':
        case 'hearth':
        case 'brazier':
          return `<path d="M ${x} ${round3(cy - a)} L ${round3(cx + a)} ${y} L ${x} ${round3(cy + a)} L ${round3(cx - a)} ${y} Z"/>`;
        case 'cage':
          return `<rect x="${round3(cx - a)}" y="${round3(cy - a)}" width="${round3(a * 2)}" height="${round3(a * 2)}"/>
            <path d="M ${round3(cx - a)} ${round3(cy - a)} L ${round3(cx + a)} ${round3(cy + a)} M ${round3(cx + a)} ${round3(cy - a)} L ${round3(cx - a)} ${round3(cy + a)}"/>`;
        case 'bunks':
        case 'table':
        case 'shelves':
          return `<rect x="${round3(cx - a * 1.3)}" y="${round3(cy - a * 0.45)}" width="${round3(a * 2.6)}" height="${round3(a * 0.9)}" rx="${round3(a * 0.15)}"/>`;
        case 'winch':
          return `<circle cx="${x}" cy="${y}" r="${b}"/><path d="M ${x} ${round3(cy - a)} V ${round3(cy + a)}"/>`;
        case 'urns':
          return `<circle cx="${round3(cx - b)}" cy="${y}" r="${round3(r * 0.4)}"/><circle cx="${round3(cx + b)}" cy="${y}" r="${round3(r * 0.4)}"/>`;
        default: // rubble
          return `<circle cx="${round3(cx - b)}" cy="${round3(cy + b * 0.3)}" r="${round3(r * 0.28)}"/>
            <circle cx="${x}" cy="${round3(cy - b)}" r="${round3(r * 0.22)}"/>
            <circle cx="${round3(cx + b)}" cy="${round3(cy + b * 0.2)}" r="${round3(r * 0.32)}"/>`;
      }
    }
    ...
    // Set dressing, in the other far corner — the chest keeps max-x/max-y.
    // No lid: these are not things you open. The shape is just enough to
    // tell a pillar from a pool at the plan's scale; the name is in the
    // paragraph.
    const dressMarks = plan.rooms.filter(lit).map((r) => {
      if (!r.dressing) return '';
      const sq = ruling && ruling.dx > 0.01 ? Math.min(ruling.dx, ruling.dy) : 2.4;
      const inset = Math.min(sq * 0.85, r.w * 0.28, r.h * 0.28);
      const cx = r.x + inset;
      const cy = r.y + r.h - inset;
      const half = Math.min(sq * 0.28, r.w * 0.14, r.h * 0.14);
      return `<g class="wm-dress is-${esc(String(r.dressing))}" aria-hidden="true">
        ${dressGlyph(String(r.dressing), cx, cy, half)}
        <title>${esc(String(r.dressing))}</title>
      </g>`;
    }).join('');
    ...
    const openingFills = openings.map((r) =>
      `<rect class="wm-opening" x="${round3(r[0])}" y="${round3(r[1])}" width="${round3(r[2] - r[0])}" height="${round3(r[3] - r[1])}"/>`
    ).join('');

    return `<g class="wm-plan">${rock}${rockwork}${walls}${floors}${rooms}${openingFills}${grid}${stairs}${doors}${propMarks}${dressMarks}${trapMarks}</g>`;
  }
  ...
  function node(n, ways, quests, marks, folk, floorplan, pinScale) {
    ...
    // Keep the pin on every named floor-plan room, including here. The party
    // stands on the room's origin; the pin sits one square north of them so
    // the faces do not cover it. Other rooms keep the pin on the origin.
    const pin = floorplan && !unknown;
    const mark = unknown
    ...
    // The tooltip hook goes on the pin cluster, not on the drawn glyph: the
    // glyph is `pointer-events: none` and the hit rect is its sibling, so a
    // hook on the glyph itself is one the mouse can never reach. The cluster
    // is what moves when the current room's pin steps one square north, so
    // the tooltip follows the i rather than the party pile.
    //
    ...
    const pinDy = pin && n.current ? round3(-(pinScale || 1) * 4) : 0;
    const pinAt = pinDy ? ` transform="translate(0 ${pinDy})"` : '';

    return `<g class="${cls}" transform="translate(${n.x} ${n.y})"
      ${n.current ? '' : `data-map-node="${n.id}" role="button" tabindex="0"`}${aria}>
