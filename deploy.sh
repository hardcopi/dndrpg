...
#
# _accent_samples: one line read by each of the speech model's non-English
# voices, generated by `gen_voiceover.py --sample-accents` so that a human can
# decide by ear whether any of them belong in the cast. Bench material for that
# decision, referenced by no page and no manifest. The clips the game actually
# plays are named for a hash of their words and sit in `assets/audio/vo/<npc>/`.
EXCLUDES=(
