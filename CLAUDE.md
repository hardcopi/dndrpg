...
- `docker/` — local stack definition (nginx vhost, PHP image, DB seeding). Not deployed.
  `docker/mapgen/` is the dungeon map service, with a vendored TypeScript
  generator under `src/dungeon/` — see **The map service**.
- `pull.sh` / `deploy.sh` — sync with production. `deploy.sh` sends `src/` only,
  so the mapgen container is a **local** thing; production runs the fallback.
...
hermes is a debugging trap. The one service that is **not** matched is `mapgen`
(Node 22) — production does not run it, on purpose, and falls back to
`DungeonGen`. Anything that depends on it must degrade, not fail. PHP extensions in `docker/php/Dockerfile` mirror the
...
## The delve — levels made at play

A **random dungeon generator**, in the manner of the 1979 DMG's Appendix A.
Written for the Undervault, which has four authored locations — a camp, a long
tent, a spoil heap, and the Mouth — and everything below them generated when a
party walks down the stair.

**It is no longer tied to that module.** A delve begins anywhere
`locations.has_delve = 1`, the module its floors are written into is read from
the mouth it began at, and `dungeon_delves.mouth_location_id` records where that
was so the party climbs back out where they went in. Two stairs exist: the
Undervault's `uv_mouth`, and `_freeplay_yard` (see **Free play**). A flag rather
than a `location_type` because the Proving Yard is already an `arena` and has to
be both — the same way `allow_camp` and `has_job_board` work.
...
- `app/lib/MapService.php` asks the **map service** for a floor, and
  `app/lib/GeneratedLevel.php` turns what comes back into the array
  `DungeonGen::generate()` returns. See **The map service** below.

`DungeonGen::finish()` is the seam between the two generators. `generate()` is
now "place the rooms, join them, finish"; `GeneratedLevel` builds its rooms and
edges from the service and calls the same `finish()`. So stocking, endpoints,
prose, passages and dressing — everything about the GAME rather than the shape
of the place — happen once, for both. A delve is the same delve whichever
generator drew it.

### The map service

`docker/mapgen` is a small Node container holding a **TypeScript dungeon
generator, vendored unmodified**. One route: `POST /generate {kind, seed, …}`.
It is on the compose network with no published port, which is why it needs no
authentication.

- **It is allowed to be absent.** `MapService::floor()` returns null on any
  failure — no `RPG_MAPGEN_URL`, container down, unreadable answer, a floor that
  could not be adapted — and `DelveEngine::draw()` falls back to `DungeonGen`.
  A missing container is a plainer dungeon, never a stair that will not go down.
  **Production has no mapgen container**, so production is the fallback path.
- **Keeps only.** The service also makes caves, wilds, towns, cities and inns.
  Caves come back as one to four chambers with no doors between them, which is a
  fine cave and a poor delve — every room here becomes a place you walk to.
- The generator carves corridors into a bitmap but never says **which rooms a
  corridor joins**, and that is what a delve is made of. `GeneratedLevel::edgesFrom()`
  reads the connectivity back out of the floor: flood-fill the non-room floor
  into components, and a component touching two or more rooms joins them,
  chained rather than all-to-all.
- Local edits to the vendored files are a last resort and are logged in
  `docker/mapgen/VENDOR.md`, because a re-vendor loses them. Anything the game
  wants belongs in the adapter or in `DungeonGen::finish()`.
- `tools/test_mapgen.php` covers it and **skips itself when there is no
  service**, because an install without the container is a supported install.
  `tools/test_delve.php` forces the fallback with `putenv('RPG_MAPGEN_URL=')`:
...
rooms, and a twist — written by `DelveEngine::writeQuest()` as ordinary `quests`
and `quest_stages` rows keyed `_dg_q_<party>_<depth>`. So the journal, the
tracker and the map markers work on it with no special case, the same way a
generated floor is ordinary `locations`.

- **No reward.** Zero XP, zero gold: the pay for a floor is the monsters
  standing on it. A quest paying on top would make the delve the only content
  worth playing.
- It **starts on descent** — there is nobody down there to hand out work.
- `QuestService::arriveAt()` advances it by walking in, and **only to the
  immediately next stage**. Matching any unpassed stage let a party walk into
  the last room first and finish the errand without doing it, which happens
  often because the tracker's own route frequently passes through the final
  room. The `_dg_q_` prefix is the whole guard that keeps this away from
  authored quests, whose location-targeted stages advance by their own effects
  and must never be moved by walking past.
- Quests are dropped with their floor. `quest_stages.target_location_id` is
  `ON DELETE SET NULL`, right for the authored world and wrong for a quest that
  exists only to point at rooms about to be deleted — the same argument
  `dropRegion()` makes about encounters.
- `tools/test_delve_quest.php` pins the walking rule, including the case that
  was wrong: standing in the last room first must not finish the errand.

**The level is stored, in `dungeon_delves.level_json`.** It used to be rebuilt
from the seed and the depth on demand, on the argument that a stored copy of a
deterministic layout is a second thing that can disagree with the first — which
is sound while the generator is a deterministic function in this repository.
It stops being sound now a floor can come from the map service, which is allowed
to be absent: a floor WRITTEN from the service and REDRAWN from the fallback
would show the player a map of a different dungeon, every wall in the right
place and every room the wrong one. The seed and depth are still stored, and
still what a `DungeonGen` floor is made of; `levelFor()` prefers the stored
level and falls back to regenerating for rows that predate the column.

**Four grids exist and share nothing**: the world is a graph of locations,
`DungeonGen` lays rooms out on a plan projected onto the chart's 0–100 × 0–75
field, `DungeonGen::tiles()` cuts that same plan into a raster of ten-foot tiles
for the first-person view, and a fight inside one of those rooms is generated by
`BattleMapGen` on its own 16×12 board exactly as a fight anywhere else is. The
raster is a sub-division of the plan rather than a fifth idea, plus a one-tile
border of rock, and `project()` is still the only conversion between plan cells
and view units.

**The plan's resolution is a property of the level, not a constant.**
`DungeonGen::gridOf($level)` returns `[grid_w ?? 9, grid_h ?? 7, sub ?? 3]`, and
`project`, `plan`, `tiles`, `routes`, `tilePoint` and their private helpers all
read it. A level that says nothing gets the old 9×7 plan and 29×23 raster, so
authored content and this generator are byte-identical; a level from the map
service sets 64×48 with `sub` 1 and is drawn at its own resolution. `ui-map.js`
needed no change — `fogTiles()` already shipped `w`/`h` with the raster.
...
## Eleven Weeks — a city, and an arrangement everybody is defending

The fourth module is **Waerhaven**, a chartered city of the Ythan League, built
against the gazetteers in [`World/`](World) — `sundermere-gazetteer.md` for the
nine realms, `waerhaven-gazetteer.md` and `waerhaven.json` for the eight wards,
forty keyed places, twelve named people and seven factions the act is authored
from. Eight regions, forty-nine locations, levels 1–6, three companions. The
design notes and the flag ledger are in [`docs/WAERHAVEN.md`](src/docs/WAERHAVEN.md);
`tools/test_waerhaven.sh` drives the whole act over real HTTP.

Three decisions in it are the kind that get undone by accident later.

- **Nobody in it is lying for gain.** Waerhaven runs an illegal population it
  depends on, held together by mutually assured embarrassments: the Moot needs
  the hands, the Watch is paid not to look, the Houses want the cheap labour,
  and the Factor cannot report the granary without reporting his own lie about
  it. Every one of them is lying because the truthful version kills somebody in
  February. Writing any of them as venal collapses the act into a hunt for a
  villain, and there is not one.
- **The party is the only body in the city with nothing in the water.** Twenty-six
  thousand people each hold something they cannot afford to have shaken; a
  stranger off the Hallowgate road holds nothing. Every major NPC hires the
  party for that reason, phrased differently, and none of them notice they are
  all saying it. It is the whole reason the act works with an outsider
  protagonist, and `tam_ryke:why_you` says it out loud once so the player hears it.
- **There are two granaries and neither knows about the other.** The Factor's
  is short because six years of grain was weighed against a standard weight
  filed four ounces light — Waerhaven has not been cheating the League, it has
  been buying air. And the Shipwrights' Moot has been quietly building a second
  one under the quay, below the high-water mark where a clause in the charter
  says the Factor's seals do not run. Both are lying, forty yards apart, for the
  same reason. That scene (`gedda:the_two_granaries`) is what the act is for.

The three endings are assembled out of other threads rather than picked off a
menu — `filled` needs the weight proved *and* the Underquay store intact *and*
a bargain with the Tarrow Houses — so a party that has only done the spine gets
told, in Culm's own voice, exactly which pieces it is missing.

**The region maps are the city's own plan, not generated art.** `World/` holds a
hand-built 2400x1800 m plan of Waerhaven with every keyed place on it at a real
coordinate, so `tools/gen_waerhaven_maps.py` crops one 4:3 window per ward out
of the SVG at native resolution — and, more importantly, writes every
location's `map_pos` back from where the place actually stands, so a chart node
sits *on* the Wet Hall rather than near it. It strips the plan's numbered key
discs, ward captions and bold gate names first, because those are set at the
weight the chart draws its own node names at and read as nodes you cannot
click; the italic street names stay, as the only thing on a cropped window that
says which way is out. Three windows are hand-set (`the_boom`, `the_slipways`,
`tarrow`) for reasons recorded in `docs/WAERHAVEN.md` — fitting them to their
buildings crops off the water that is the whole point of them.

## Free play — the adventures, put away

`ADVENTURES_ENABLED` in `app/bootstrap.php` is **false** locally. Off means the
game is characters and random encounters and nothing else: the shelf is not
drawn, Play does not open a module, creation does not ask which world, and
`session/modules` answers with an empty list so a stale page cannot start a game
in a hidden module either.

- **The Proving Yard** (`_freeplay_yard`) is where everyone stands. It is an
  `arena`, so it offers the pit's three tiers, and it has `has_delve = 1`, so it
  offers the stair. One place, two doors.
- It hangs off a module `_freeplay`, which exists only because
  `regions.module_id` is NOT NULL and a place has to hang off something.
  `is_active = 0` keeps it off the shelf permanently. It is not a game to choose.
- `CharacterGenerator::resolveModule()` checks the flag **after the party and
  before the request**, and the order matters. A party already inside a module
  lives there — that is also how a defeated party finds the right haven, and put
  ahead of the party lookup it marched everybody who lost a fight in the
  Undervault out to the Proving Yard. A request that NAMES a module is
  overruled, so an old bookmark cannot put a new character into a hidden game.
- Turning the adventures back on is that one constant. **What it cannot undo is
  the parties' module links**, wiped deliberately on the dev database (see
  `backups/rpg_5e-before-module-wipe-*.sql`). **Production's were not touched.**

The Random Encounter row that used to sit on the front-page sheet is gone; the
yard's own TAKE A BOUT panel asks the same question where the fight happens.
...
## The 3D character creator

A second way to look like somebody, beside the portraits and the paperdoll: a
Unity WebGL build of Synty's Sidekick modular characters, embedded in a page.

- **The Unity project is a separate repository**, `/home/richard/code/rpg-unity`.
  Build it with `./build-embed.sh`, which produces a player plus AssetBundles and
  is deployed by copying that folder to `src/embed/`. Read that project's
  `README.md` and `EMBED.md` first — it has traps of its own, notably that
  adding a folder under `Assets/` hangs the editor, and that headless Unity must
  run as `richard` or it exits 0 having built nothing.
- `src/embed/` — the built player. `src/look.php` hosts it for one character;
  `src/embed_demo.php` shows the three ways to embed it;
  `src/assets/js/rivermark-character.js` is the host-side module, the
  `<rivermark-character>` element, and the postMessage bridge.
- **The recipe** is `characters.sidekick_json`, validated by
  `app/lib/SidekickAppearance.php` and read and written through
  `character/model` (GET and POST). Not `character/appearance` — that is the 2D
  paperdoll, and the two are different pictures of the same person.
- **The recipe's key order is load-bearing.** `v, species, race, parts, shape,
  skin, outfit, colors`, matching the C# field order exactly, because the
  creator decides whether it has unsaved work by comparing its recipe with the
  server's byte for byte. `parts` and `colors` are sorted on both sides.
  `tools/` has no coverage of this; the Unity project's `Tools/recipe-test.sh`
  does.
- **Race and species are both stored, because neither derives from the other.**
  Six races share the human species, so a recipe holding only the species could
  not tell a dwarf from a halfling coming back. `character/model` returns the
  character's race so the creator opens as whoever they already are, and the
  race picker is not drawn when the race came from a sheet — otherwise a saved
  recipe could say Elf while `characters.race` says Dragonborn, and the portrait
  and the sheet would disagree about what somebody is.
- The creator is also the third tab of the **Appearance** step in `create.php`,
  mounted only when that tab is first opened — it is a WebGL player and forty
  megabytes of meshes, and most people making a character never touch it.
...
**`combat/random` deals the party the fighting pit's own match wherever they are
standing** — `PitEngine::skirmish()`, then the ordinary `startEncounter`.

The *Random encounter* row that used to offer this on the front-page sheet has
been **removed**: the Proving Yard's own TAKE A BOUT panel offers the same three
tiers where the fight actually happens, and two controls for one thing is one
too many. The route stays, and everything below still describes it.
...
- **`deploy.sh` needs richard's SSH key**, and an agent running as root has
  `$HOME=/root` where there is none. Use `sudo -u richard ./deploy.sh`. For the
  production database, `mysql` on hermes authenticates root over the socket, so
  nothing needs a password on a command line.
- **`sql/` is NOT excluded from `deploy.sh`,** despite the comment above the
  `EXCLUDES` array saying schema files are applied deliberately rather than
  swept along. The migrations are published to the public web root. They are
  inert as files, but it publishes the schema.
- **Migrations are never run by a deploy.** `deploy.sh` copies files; it does
  not apply them. Anything with a new column must be migrated on hermes first or
  the live site 500s — `LocationEngine::getState()` asks for the delve status on
  every look, so a missing `locations.has_delve` breaks every game screen. Every
  migration in `src/sql/migrations/` is written to be safe to run twice.
- **`asset()` is not a legal ES module specifier.** It returns a bare relative
  path, which is fine for `<link>` and `<script src>` and refused by
  `import`. A dynamic `import()` inside a classic script also resolves against
  the SCRIPT's URL, not the page's — so `./assets/js/x.js` from a file already in
  `/assets/js/` asks for `/assets/js/assets/js/x.js`. Resolve through
  `new URL(asset('...'), document.baseURI)`.
- **`tools/test_delve.php` has one flaky assertion** — "somebody paid for it, or
  saved well" fails perhaps one run in five, because it asserts on a saving
  throw. It predates the delve being freed from the Undervault.
